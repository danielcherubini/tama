...
//! `koji` data directory.
//!
//! When the project was renamed from `kronk` to `koji`, existing users had
//! data under `~/.config/kronk` (Linux) or `%APPDATA%\kronk` (Windows). On
//! first run of the new binary we rename that directory to the `koji`
...
/// Migrate a legacy kronk data directory to the new koji location if needed.
///
...
pub fn migrate_legacy_data_dir(new_dir: &Path) -> anyhow::Result<Option<Migration>> {
    ...
    if legacy_db.exists() {
        let new_db = new_dir.join("koji.db");
        if !new_db.exists() {
...
mod tests {
    ...
    fn test_noop_when_new_dir_already_exists() {
        ...
        let new_dir = tmp.path().join("koji");
    ...
    fn test_noop_when_no_legacy_dir_present() {
        ...
        // does.
        let tmp = tempdir().unwrap();
        let new_dir = tmp.path().join("koji-does-not-exist");
    ...
    fn test_rename_moves_directory_and_renames_db() {
        ...
        std::fs::rename(&legacy_db, new_dir.join("koji.db")).unwrap();

        assert!(new_dir.join("koji.db").exists());
        assert!(!new_dir.join("kronk.db").exists());
        assert_eq!(
            fs::read(new_dir.join("koji.db")).unwrap(),
