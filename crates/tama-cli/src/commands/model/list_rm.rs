    ...
    match model_id_arg {
        None => {
            if model_configs.is_empty() {
                println!("No models configured. Run `tama model pull <repo>` to add one.");
                return Ok(());
            }

            let mut entries: Vec<(&String, &tama_core::config::ModelConfig)> =
        ...
        Some(model_id) => {
            ...
            let mc = model_configs.get(&model_id).with_context(|| {
                format!(
                    "Model config '{}' not found. Run `tama model ls` to see configured models.",
                    model_id
                )
            })?;
            ...
            if !mc.quants.is_empty() {
                ...
                for (qname, qe) in quants {
                    ...
                    let path = tama_core::models::repo_path(&models_dir, repo).join(&qe.file);
