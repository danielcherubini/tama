...
pub(super) async fn cmd_update(
    ...
    let models: Vec<tama_core::models::InstalledModel> = match model_filter {
        Some(ref id) => {
            let found = registry
                .find(id)?
    ...
    let models_to_update: Vec<(
        ...
        &tama_core::models::InstalledModel,
    )> = check_results
        .iter()
        .zip(models.iter())
    ...
    for (result, model) in &models_to_update {
        ...
        let listing = tama_core::models::pull::list_gguf_files(repo_id).await?;
        let blobs = tama_core::models::pull::fetch_blob_metadata(&listing.repo_id)
            .await
            .ok();
...
pub(super) async fn cmd_search(
    ...
    if pull {
        ...
        println!("  Pull one:  tama model pull <model-id>");
    }
