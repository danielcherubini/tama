...
pub(super) async fn cmd_pull(config: &Config, repo_id: &str) -> Result<()> {
    ...
    for display_str in &selected {
        ...
        card.quants.insert(
            ...
            QuantInfo {
                ...
                kind: tama_core::config::QuantKind::from_filename(&gguf.filename),
    ...
    let vram = tama_core::gpu::query_vram();

    let selected_ctx = if largest_model_bytes > 0 {
        let suggestions = tama_core::gpu::suggest_context_sizes(largest_model_bytes, vram.as_ref());
    ...
    for quant_key in card.quants.keys() {
        println!(
            "    tama model create --model {} --quant {} --profile coding --name my-server",
            model_id, quant_key
        );
    }
...
mod tests {
    ...
    use tama_core::db::{open, OpenResult};
    ...
    async fn test_scan_adds_new_files() {
        ...
        let files = tama_core::db::queries::get_model_files(conn, model_id).unwrap();
