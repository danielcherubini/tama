...
pub fn win_service_main(_arguments: Vec<std::ffi::OsString>) {
    ...
    let log_dir = config_dir
        .clone()
        .unwrap_or_else(|| {
            directories::ProjectDirs::from("", "", "tama")
    ...
    let log_file = std::fs::File::create(log_dir.join(format!("{}.log", service_name)))
        .or_else(|_| std::fs::File::create("tama-service.log"))
        .or_else(|_| std::fs::File::create(std::env::temp_dir().join("tama-service.log")))
        .or_else(|_| {
    ...
    rt.block_on(async {
        if is_proxy {
            ...
            tracing::info!("Starting Tama proxy service on {}", addr);
            ...
            #[cfg(feature = "web-ui")]
            {
                ...
                let config_path = config_dir
                    ...
                    .or_else(|| tama_core::config::Config::config_path().ok());
                ...
                let capabilities =
                    std::sync::Arc::new(tama_web::api::backends::CapabilitiesCache::new());
            ...
            let model_configs = match tama_core::db::load_model_configs(&conn) {
                Ok(c) => c,
                Err(e) => {
...
// On Windows, use the real ProcessSupervisor from tama_core
#[cfg(target_os = "windows")]
