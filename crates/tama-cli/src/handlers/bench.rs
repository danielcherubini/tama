...
//!
//! Handles `tama bench` for benchmarking model inference performance.

use anyhow::{bail, Context, Result};
use tama_core::bench::{self, BenchConfig};
...
pub async fn cmd_bench(
    ...
    let server_names: Vec<String> = if all {
        ...
        if servers.is_empty() {
            bail!("No enabled model configs found. Create one with `tama model create`.");
        }
