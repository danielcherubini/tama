...
/// Add a new server from a command line, extracting tama flags.
pub async fn cmd_server_add(
    ...
    let registry =
        tama_core::models::ModelRegistry::new(models_dir.to_path_buf(), configs_dir.to_path_buf());
    ...
    let model_info = if let Some(ref model_ref) = extracted.model {
        match registry.find(model_ref) {
            ...
            Ok(None) => {
                anyhow::bail!(
                    "Model '{}' not found. Use `tama model pull <repo>` to install it first.",
                    model_ref
                );
            }
            Err(e) => {
    ...
    let quant_name = if let Some(ref quant) = extracted.quant {
        ...
        if quant_names.is_empty() {
            anyhow::bail!(
                "No quants available for '{}'. Run `tama model pull {}` first.",
                installed.card.model.source,
                installed.card.model.source
            );
        }
    ...
    println!();
    println!("Enable it:  tama model enable {}", name);
