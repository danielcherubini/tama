...
pub fn cmd_server_rm(config: &Config, name: &str, force: bool) -> Result<()> {
    ...
    if service_installed {
        anyhow::bail!(
            "Server '{}' has an installed service '{}'. Remove it first with: tama service remove {}",
    ...
    if let Some(id) = model_id {
        tama_core::db::queries::delete_model_config(&conn, id)?;
    }
...
mod tests {
    ...
    fn test_server_rm_deletes_by_config_name_not_repo_id() {
        ...
        let id_a = tama_core::db::save_model_config(&conn, "org--model-a", &mc_a).unwrap();
        let id_b = tama_core::db::save_model_config(&conn, "org--model-b", &mc_b).unwrap();

        // Verify both exist before deletion.
        let configs = tama_core::db::load_model_configs(&conn).unwrap();
        ...
        tama_core::db::queries::delete_model_config(&conn, id_a).unwrap();
    ...
    fn test_repo_id_comparison_would_fail() {
        ...
        let id = tama_core::db::save_model_config(&conn, "org--model-a", &mc).unwrap();

        // The original buggy code would do:
        //   all_configs.iter().find(|c| c.repo_id == name)
        // where `name` is the config key "org--model-a".
        let all_configs = tama_core::db::queries::get_all_model_configs(&conn).unwrap();
