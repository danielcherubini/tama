...
//!
//! Handles `tama status` for showing status of all servers.
...
fn resolve_model_contexts(config: &Config) -> std::collections::HashMap<String, u64> {
    ...
    if let (Some(models_dir), Some(configs_dir)) = (models_dir, configs_dir) {
        let registry =
            tama_core::models::ModelRegistry::new(models_dir.clone(), configs_dir.clone());

        let db_dir = tama_core::config::Config::config_dir().ok();
        let model_configs = db_dir
            .as_ref()
            .and_then(|dir| {
                tama_core::db::open(dir)
                    .ok()
                    .map(|r| tama_core::db::load_model_configs(&r.conn).unwrap_or_default())
            })
            .unwrap_or_default();
...
fn print_offline_status(config: &Config) -> Result<()> {
    if let Some(vram) = tama_core::gpu::query_vram() {
        println!("  VRAM:     {} / {} MiB", vram.used_mib, vram.total_mib);
    }

    let db_res = (|| -> Result<(tama_core::db::Connection, Vec<tama_core::db::queries::ActiveModelRecord>, std::collections::HashMap<String, tama_core::config::ModelConfig>)> {
        let db_dir = tama_core::config::Config::config_dir()?;
        let OpenResult { conn, .. } = tama_core::db::open(&db_dir)?;
        let db_active = tama_core::db::queries::get_active_models(&conn)?;
    ...
    for (name, srv) in &model_configs {
        ...
        let status_str = match db_entry {
            Some(active) => {
                let pid = active.pid;
                if tama_core::proxy::process::is_process_alive(pid as u32) {
