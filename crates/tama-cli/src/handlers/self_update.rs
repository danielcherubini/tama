...
//!
//! Checks for and installs updates to the tama binary from GitHub releases.

use anyhow::Result;

/// Handle the `tama self-update` command.
///
...
pub async fn cmd_self_update(check: bool, force: bool) -> Result<()> {
    ...
    let info = tama_core::self_update::check_for_update(current_version).await?;
    ...
    if check {
        if info.update_available {
            println!("\nUpdate available! Run `tama self-update` to install.");
        }
        return Ok(());
    }
    ...
    let result =
        tama_core::self_update::perform_update(current_version, |msg| println!("  {}", msg))
            .await?;
    ...
    println!("Please restart tama to use the new version.");
