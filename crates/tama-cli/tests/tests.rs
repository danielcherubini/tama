/// Tests for extract_tama_flags helper function
use tama::extract_tama_flags;

#[test]
fn test_extract_model_huggingface_ref() {
    let args = vec!["--model".to_string(), "unsloth/Qwen3.5-0.8B".to_string()];
    let result = extract_tama_flags(args).unwrap();
...
#[test]
fn test_mixed_tama_and_backend_flags() {
    let args = vec![
        "--model".to_string(),
...
#[test]
fn test_no_tama_flags_all_remain() {
    let args = vec![
        "--host".to_string(),
        "0.0.0.0".to_string(),
        "-t".to_string(),
...
fn test_model_without_value_returns_error() {
    ...
    let result = extract_tama_flags(args);
...
use tama::{cmd_server_add, cmd_server_edit};
...
async fn test_cmd_server_edit_valid_profile_succeeds() {
    ...
    {
        let tama_core::db::OpenResult { conn, .. } =
    ...
    {
        let tama_core::db::OpenResult { conn, .. } =
            tama_core::db::open(db_dir).expect("Failed to open DB for cleanup");
        // Look up model_id by repo_id for cleanup
        if let Some(record) =
            tama_core::db::queries::get_model_config_by_repo_id(&conn, "test_server")
                .ok()
                .flatten()
        {
            tama_core::db::queries::delete_model_config(&conn, record.id).ok();
        }
    }
