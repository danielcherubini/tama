...
#[cfg(feature = "ssr")]
use crate::components::{BackendCard, InstallModal, JobLogPanel, InstallRequest, GpuTypeDto, CapabilitiesDto};
...
// ─── Backends Form with new components ────────────────────────────────────
...
fn BackendsForm(config: RwSignal<Option<Config>>) -> impl IntoView {
    ...
    let (selected_backend, set_selected_backend) = create_signal::<Option<String>>(None);
    
    // Job log panel state
    let (show_job_log, set_show_job_log) = create_signal(false);
    let (current_job_id, set_current_job_id) = create_signal(String::new());
    
    // Cache for last-submitted install requests
    let install_requests: RwSignal<HashMap<String, InstallRequest>> = RwSignal::new(HashMap::new());
    
    // Loopback warning banner state
    let (show_loopback_banner, set_show_loopback_banner) = create_signal(false);
    
    // Check for loopback warning on mount
    Effect::new(move |_| {
        spawn_local(async move {
            let bound_to_loopback = std::env::var("KOJI_BOUND_TO_LOOPBACK")
                .map(|v| v == "true")
                .unwrap_or(true);
            set_show_loopback_banner.set(!bound_to_loopback);
        });
    });
    
    // Handle install click
    let on_install_click = move |backend_name: String| {
        set_selected_backend.set(Some(backend_name.clone()));
        set_show_install_modal.set(true);
    };
    
    // Handle delete click
    let on_delete_click = move |backend_name: String, event: MouseEvent| {
        event.prevent_default();
        spawn_local(async move {
            let res = gloo_net::http::Request::delete(&format!("/api/backends/{}", backend_name))
                .send()
                .await;
            match res {
                Ok(resp) => {
                    if resp.ok() {
                        if let Ok(resp) = gloo_net::http::Request::get("/api/config/structured").send().await {
                            if let Ok(cfg) = resp.json::<Config>().await {
                                config.set(Some(cfg));
                            }
                        }
                    } else {
                        let text = resp.text().await.unwrap_or_default();
                        leptos::logging::log!("Failed to delete: {}", text);
                    }
                }
                Err(e) => leptos::logging::log!("Failed to delete backend: {}", e),
            }
        });
    };
    
    // Handle update click
    let on_update_click = move |backend_name: String| {
        spawn_local(async move {
            let res = gloo_net::http::Request::post(&format!("/api/backends/{}/update", backend_name))
                .send()
                .await;
            match res {
                Ok(resp) => {
                    if resp.ok() {
                        if let Ok(job_id) = resp.text().await {
                            leptos::logging::log!("Update started, job id: {}", job_id);
                        }
                        if let Ok(resp) = gloo_net::http::Request::get("/api/config/structured").send().await {
                            if let Ok(cfg) = resp.json::<Config>().await {
                                config.set(Some(cfg));
                        ...
                        let text = resp.text().await.unwrap_or_default();
                        leptos::logging::log!("Update failed: {}", text);
                    }
                }
                Err(e) => leptos::logging::log!("Failed to check/update backend: {}", e),
            }
        });
    };
    
    // Handle install modal close
    let on_install_modal_close = move |_| {
        set_show_install_modal.set(false);
        set_selected_backend.set(None);
    };
    
    // Handle job log panel close
    let on_job_log_close = move |_| {
        set_show_job_log.set(false);
        set_current_job_id.set(String::new());
    };
    ...
    view! {
        <div class="card">
            <h2>"Backends"</h2>
            <p class="text-muted">"Inference backend binaries and default arguments."</p>
            
            {/* Loopback warning banner */}
            {move || {
                if show_loopback_banner() {
                    view! {
                        <div class="alert alert-warning" style="margin-bottom: 1rem;">
                            <span>"⚠ Warning: The web UI is bound to a non-loopback address. Install/update operations require same-origin requests."</span>
                        </div>
                    }.into_any()
                } else {
                    view! { <></> }.into_any()
                    ...
                    view! {
                        <div style="display:flex;flex-direction:column;gap:1.5rem;margin-top:1rem;">
                            {keys.into_iter().map(|key| {
                                let k_install = key.clone();
                                let k_delete = key.clone();
                                let k_update = key.clone();
                                view! {
                                    <BackendCard
                                        name=key.clone()
                                        config=config
                                        on_install=move || on_install_click(k_install.clone())
                                        on_update=move || on_update_click(k_update.clone())
                                        on_delete=move |ev| on_delete_click(k_delete.clone(), ev)
                                    />
                                }.into_any()
                            }).collect::<Vec<_>>()}
                        </div>
            ...
            {move || {
                if show_install_modal() {
                    if let Some(backend_name) = selected_backend() {
                        view! {
                            <InstallModal
                                visible=show_install_modal
                                backend_type=Signal::derive(backend_name.clone())
                                on_submit=EventHandler::new(move |req| {
                                    install_requests.update(|map| {
                                        map.insert(backend_name.clone(), req.clone());
                                    });
                                    leptos::logging::log!("Install requested for {}: {:?}", backend_name, req);
                                })
                                on_cancel=EventHandler::new(on_install_modal_close)
                            />
                        }.into_any()
                    }
                } else {
                    ...
                    if !job_id.is_empty() {
                        view! {
                            <JobLogPanel
                                show=show_job_log
                                job_id=job_id
                                on_close=EventHandler::new(on_job_log_close)
                            />
                        }.into_any()
                    } else {
                        view! { <></> }.into_any()
...
#[component]
fn BackendsForm(config: RwSignal<Option<Config>>) -> impl IntoView {
    // Fallback for non-SSR builds - use old implementation
    let backend_keys = move || {
        config
            .get()
            .map(|c| c.backends.keys().cloned().collect::<Vec<_>>())
