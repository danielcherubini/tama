//! Self-update functionality for the Tama binary.
//!
...
/// GitHub repository name for Tama releases.
...
/// Download and install the latest Tama release, replacing the running binary.
///
...
mod tests {
    ...
    fn test_detect_archive_kind_tar_gz() {
        let kind = detect_archive_kind("tama-x86_64-unknown-linux-gnu.tar.gz");
        assert!(matches!(
            kind,
            self_update::ArchiveKind::Tar(Some(self_update::Compression::Gz))
        ));
    ...
    fn test_detect_archive_kind_tgz() {
        let kind = detect_archive_kind("tama-linux-amd64.tgz");
        assert!(matches!(
            kind,
            self_update::ArchiveKind::Tar(Some(self_update::Compression::Gz))
        ));
    ...
    fn test_detect_archive_kind_plain_binary() {
        let kind = detect_archive_kind("tama");
    ...
    fn test_detect_archive_kind_plain_with_extension() {
        ...
        let kind = detect_archive_kind("tama.bin");
        assert!(matches!(kind, self_update::ArchiveKind::Plain(None)));
    }
    ...
    fn test_detect_archive_kind_with_path() {
        ...
        let kind = detect_archive_kind("/tmp/releases/v1.0/tama.tar.gz");
        assert!(matches!(
            kind,
            self_update::ArchiveKind::Tar(Some(self_update::Compression::Gz))
        ));
