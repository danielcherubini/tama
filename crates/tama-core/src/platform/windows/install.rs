...
/// Install tama as a native Windows Service for the given server.
/// The service will run `tama.exe service-run --server <name> --config-dir <path>` when started.
...
/// Install tama proxy as a native Windows Service.
/// The service will run `tama.exe service-run --proxy --config-dir <path>` when started.
pub fn install_proxy_service(config_dir: &std::path::Path, port: u16) -> Result<()> {
    let exe_path = std::env::current_exe().context("Failed to get current exe path")?;
    let service_name = "tama";
    let display_name = "Tama";
    ...
    super::firewall::add_firewall_rule(service_name, port).ok();
    super::firewall::add_firewall_rule("tama-web", 11435).ok();
    super::permissions::grant_user_control(service_name)
        .with_context(|| format!("Failed to set service permissions for '{}'", service_name))?;
