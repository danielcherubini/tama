...
/// If the DB has no model_configs rows but tama.toml contains a [models] section,
...
pub fn migrate_models_to_db(conn: &Connection, config: &mut Config) -> anyhow::Result<usize> {
    ...
    tracing::info!(
        "Migrating {} models from tama.toml to database",
        migrated_count
    );
    ...
    // Since Config.models is gone, this will overwrite tama.toml WITHOUT the [models] section.
    config.save()?;
