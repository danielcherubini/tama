...
fn test_kv_cache_type_args() {
    ...
    // --- llama.cpp backend with cache types set ---
    let server = ModelConfig {
        backend: "llama_cpp".to_string(),
        args: vec![],
        sampling: None,
    ...
    // --- llama.cpp backend with cache types None ---
    let server_none = ModelConfig {
        cache_type_k: None,
        cache_type_v: None,
        quants: quants.clone(),
        ..server.clone()
    ...
    let args_none = config
        .build_full_args(&server_none, &backend, None)
        .expect("build_full_args failed");

    // -ctk and -ctv should NOT be present
    assert!(
        !args_none.iter().any(|a| *a == "-ctk"),
        "Expected no -ctk when cache_type_k is None, got: {:?}",
        args_none
    );
    assert!(
        !args_none.iter().any(|a| *a == "-ctv"),
        "Expected no -ctv when cache_type_v is None, got: {:?}",
        args_none
    );

    // --- non-llama.cpp backend with cache types set ---
    let server_non_llama = ModelConfig {
        backend: "ollama".to_string(),
        quants: quants.clone(),
        ..server.clone()
    };

    let args_non_llama = config
        .build_full_args(&server_non_llama, &backend, None)
        .expect("build_full_args failed");

    // -ctk and -ctv should NOT be present for non-llama.cpp backends
    assert!(
        !args_non_llama.iter().any(|a| *a == "-ctk"),
        "Expected no -ctk for non-llama.cpp backend, got: {:?}",
        args_non_llama
    );
    assert!(
        !args_non_llama.iter().any(|a| *a == "-ctv"),
        "Expected no -ctv for non-llama.cpp backend, got: {:?}",
        args_non_llama
    );

    // --- llama.cpp backend with cache types already in args (should not duplicate) ---
    let server_dup = ModelConfig {
    ...
    let args_dup = config
        .build_full_args(&server_dup, &backend, None)
        .expect("build_full_args failed");

    // -ctk should appear exactly once (from args, not injected)
    let ctk_count = args_dup.iter().filter(|a| *a == "-ctk").count();
    assert_eq!(
        ctk_count, 1,
    ...
    let ctv_count = args_dup.iter().filter(|a| *a == "-ctv").count();
    assert_eq!(
        ctv_count, 1,
    ...
    // --- llama.cpp backend with empty string cache types (should not inject) ---
    let server_empty = ModelConfig {
    ...
    let args_empty = config
        .build_full_args(&server_empty, &backend, None)
        .expect("build_full_args failed");

    assert!(
        !args_empty.iter().any(|a| *a == "-ctk"),
        "Expected no -ctk when cache_type_k is empty string, got: {:?}",
        args_empty
    );
    assert!(
        !args_empty.iter().any(|a| *a == "-ctv"),
