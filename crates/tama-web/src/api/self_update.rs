//! Self-update API endpoints for the Tama web UI.
//!
//! Provides three endpoints:
//! - `GET /tama/v1/self-update/check` — check if a new version is available
//! - `POST /tama/v1/self-update/update` — trigger the update (CSRF-protected)
...
/// Response for `POST /tama/v1/self-update/update`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn check_update(
    ...
    match tama_core::self_update::check_for_update(&state.binary_version).await {
    ...
    tokio::spawn(async move {
        ...
        match tama_core::self_update::perform_update(&binary_version, progress_callback).await {
            Ok(result) => {
                ...
                if let Err(e) = tama_core::self_update::restart_process() {
