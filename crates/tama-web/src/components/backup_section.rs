...
pub fn BackupSection() -> impl IntoView {
    ...
    let backup_handler = move |_| {
        ...
        spawn_local(async move {
            match Request::get("/tama/v1/backup").send().await {
                Ok(resp) => {
                                    ...
                                    Ok(url) => {
                                        if let Some(doc) = window.document() {
                                            if let Ok(a) = doc.create_element("a") {
                                                ...
                                                let _ = a.set_attribute("download", "tama-backup.tar.gz");
                                                a.click();
    ...
    let restore_handler = move |_| {
        ...
        spawn_local(async move {
            ...
            let build_result = match post_request("/tama/v1/restore").json(&body) {
                Ok(r) => r,
                Err(e) => {
                    set_restoring.set(false);
                        ...
                        view! {
                            <div class="border rounded-lg p-4 mb-4 bg-gray-50">
                                <div class="text-sm text-gray-600 mb-2">
                                    ...
                                    <p><strong>"Tama version:"</strong> {preview.tama_version}</p>
