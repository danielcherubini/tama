...
pub struct Job {
    pub id: JobId,
    pub kind: JobKind,
    pub backend_type: Option<tama_core::backends::BackendType>,
...
impl JobManager {
    ...
    pub async fn submit(
        &self,
        kind: JobKind,
        backend_type: Option<tama_core::backends::BackendType>,
...
mod tests {
    ...
    async fn test_submit_then_finish_transitions_state() {
        ...
        let job = manager
            .submit(
                JobKind::Install,
                Some(tama_core::backends::BackendType::LlamaCpp),
            )
            .await
    ...
    async fn test_concurrent_submit_returns_already_running() {
        ...
        let result = manager
            .submit(
                JobKind::Update,
                Some(tama_core::backends::BackendType::IkLlama),
            )
            .await;
