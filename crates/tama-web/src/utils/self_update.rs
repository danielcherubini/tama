...
pub async fn stream_update_events(
    ...
    let mut es = match EventSource::new("/tama/v1/self-update/events") {
        Ok(es) => es,
        Err(e) => {
            ...
            futures_util::future::Either::Right((Some(Ok((_, msg))), _)) => {
                ...
                if let Ok(payload) = serde_json::from_str::<StatusPayload>(&data) {
                    match payload.status.as_str() {
                        ...
                        "restarting" => {
                            update_status.set("Restarting Tama...".to_string());
                            update_succeeded = true;
                            break;
                        }
                        _ => {}
...
/// Poll `/tama/v1/self-update/check` every 2 seconds until the server
