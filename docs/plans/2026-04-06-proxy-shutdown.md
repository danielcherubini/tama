...
**Tech Stack:** Rust, Tokio, Tama Core.
...
**Context:**
Currently, `ProxyState` manages the lifecycle of models and metrics but lacks a centralized way to shut down all active components. This is needed for a graceful shutdown of the Tama proxy.

**Files:**
- Modify: `crates/tama-core/src/proxy/lifecycle.rs` (to add the implementation)
- Modify: `crates/tama-core/src/proxy/mod.rs` (to add tests)

**What to implement:**
Add `async fn shutdown(&self)` to `impl ProxyState` in `crates/tama-core/src/proxy/lifecycle.rs`.
The method must:
...
**Steps:**
- [ ] Write failing test for `ProxyState::shutdown` in `crates/tama-core/src/proxy/mod.rs`
- [ ] Run `cargo test --package tama-core`
  - Did it fail with `method not found`? If it passed unexpectedly, stop and investigate why.
- [ ] Implement `shutdown` in `crates/tama-core/src/proxy/lifecycle.rs`
- [ ] Run `cargo test --package tama-core`
