...
**Goal:** Make `server add` and `server edit` extract kronk-specific flags (`--model`, `--profile`, `--quant`, `--port`, `--ctx`) from raw args into proper `ModelConfig` struct fields, with interactive quant selection and model card validation.
**Architecture:** Add an `extract_kronk_flags()` helper that parses the raw arg vec, separating kronk flags from backend passthrough args. Refactor `cmd_server_add` and `cmd_server_edit` to use it. Extract shared backend resolution logic into `resolve_backend()`.
**Tech Stack:** Rust, clap (existing), inquire (existing), kronk-core ModelRegistry

---

### Task 1: Add `extract_kronk_flags()` helper with tests
...
- [ ] Write tests for `extract_kronk_flags()` in `tests.rs`:
  ...
  - Mixed kronk and backend flags separated correctly
  - No kronk flags → all args remain as backend args
...
- [ ] Implement `ExtractedFlags` struct and `extract_kronk_flags()` function in `main.rs` with doc comments:
  ```rust
  ...
  fn extract_kronk_flags(args: Vec<String>) -> Result<ExtractedFlags>
  ```
...
- [ ] Refactor `cmd_server_add` to:
  ...
  3. Call `extract_kronk_flags()` on the args
...
- [ ] Write a test for `cmd_server_edit` that verifies:
  ...
  - Editing with only backend args preserves all kronk fields
...
- [ ] Refactor `cmd_server_edit` to:
  ...
  2. Split command, call `resolve_backend()` and `extract_kronk_flags()`
...
- [ ] Run `cargo clippy --workspace -- -D warnings`
- [ ] Run `cargo test --workspace`
- [ ] Manual verification: run `kronk server add test_server llama-server --model unsloth/Qwen3.5-0.8B --profile chat` and inspect config.toml output
