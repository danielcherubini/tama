...
- The DB path is resolved via `Config::config_dir()?` (static method returning `~/.config/kronk/`).
...
**Context:**
Kronk currently has no database — all state lives in TOML files and the filesystem. We need a SQLite database at `~/.config/kronk/kronk.db` to store internal metadata that doesn't belong in user-editable TOML cards (commit hashes, file hashes, timestamps, download logs). This task sets up the foundation: the dependency, the module structure, a connection helper, and an automatic migration system. The migration system uses SQLite's `PRAGMA user_version` to track schema version.

**Files:**
- Modify: `Cargo.toml` (workspace root — add rusqlite to workspace dependencies)
- Modify: `crates/kronk-core/Cargo.toml` (add rusqlite dependency)
- Create: `crates/kronk-core/src/db/mod.rs` (connection helper, migration runner)
- Create: `crates/kronk-core/src/db/migrations.rs` (versioned SQL migrations)
- Modify: `crates/kronk-core/src/lib.rs` (add `pub mod db;`)
...
2. **`db/mod.rs`** — Public API:
   - `pub mod migrations;`
   - `pub fn open(config_dir: &Path) -> Result<Connection>` — Opens (or creates) `config_dir/kronk.db`, enables WAL mode (`PRAGMA journal_mode=WAL`), enables foreign keys (`PRAGMA foreign_keys=ON`), runs migrations, returns the connection.
...
- [ ] Add `rusqlite.workspace = true` to `crates/kronk-core/Cargo.toml` under `[dependencies]`
- [ ] Create `crates/kronk-core/src/db/mod.rs` with `open()`, `open_in_memory()` functions
- [ ] Create `crates/kronk-core/src/db/migrations.rs` with the migration runner and v1 schema
- [ ] Add `pub mod db;` to `crates/kronk-core/src/lib.rs`
...
- [ ] Run `cargo test --package kronk-core -- db::tests`, confirm tests pass
...
**Acceptance criteria:**
- [ ] `kronk.db` is created in the config directory when `db::open()` is called
...
**Files:**
- Create: `crates/kronk-core/src/db/queries.rs`
- Modify: `crates/kronk-core/src/db/mod.rs` (add `pub mod queries;`)
...
**Steps:**
- [ ] Create `crates/kronk-core/src/db/queries.rs` with all functions and structs above
- [ ] Add `pub mod queries;` to `crates/kronk-core/src/db/mod.rs`
...
- [ ] Run `cargo test --package kronk-core -- db`, confirm all pass
- [ ] Run `cargo fmt --all && cargo clippy --workspace -- -D warnings`
...
### Task 3: Record metadata in DB during `kronk model pull`
...
**Files:**
- Modify: `crates/kronk-core/src/models/pull.rs` (return commit SHA, add blob metadata fetching)
- Modify: `crates/kronk-cli/src/commands/model.rs` (open DB in cmd_pull, record metadata)
...
2. **`cmd_pull()` changes in `model.rs`:**
   ...
   - Open the DB: `let db_dir = kronk_core::config::Config::config_dir()?;`
...
- [ ] Run `cargo test --package kronk-core -- pull`, confirm tests pass
...
- [ ] After `kronk model pull`, the DB contains the repo's commit SHA and per-file LFS SHA256 hashes
...
**Files:**
- Create: `crates/kronk-core/src/models/update.rs`
- Modify: `crates/kronk-core/src/models/mod.rs` (add `pub mod update;`)
...
**Steps:**
- [ ] Create `crates/kronk-core/src/models/update.rs` with all types and functions above
- [ ] Add `pub mod update;` to `crates/kronk-core/src/models/mod.rs`
- [ ] Write unit tests:
...
- [ ] Run `cargo test --package kronk-core -- models::update`, confirm tests pass
- [ ] Run `cargo fmt --all && cargo clippy --workspace -- -D warnings`
...
### Task 5: Add `kronk model update` CLI command

**Context:**
This is the user-facing command that ties everything together. It supports three modes: (1) `kronk model update` — check all installed models for updates, (2) `kronk model update <model>` — check a specific model, (3) `kronk model update --check` — dry-run that only reports status without downloading. There's also `kronk model update --refresh` to stamp existing models with metadata without re-downloading. The `--check` and `--refresh` flags are mutually exclusive. The command opens the DB, iterates models, calls `check_for_updates()`, displays results, and optionally re-downloads changed files.

**Files:**
- Modify: `crates/kronk-cli/src/cli.rs` (add `Update` variant to `ModelCommands`)
- Modify: `crates/kronk-cli/src/commands/model.rs` (add `cmd_update()` function, add match arm)
...
3. **`cmd_update()` in `model.rs`:**
   ...
   Logic:
   1. Open DB: `let db_dir = Config::config_dir()?;` then `let conn = kronk_core::db::open(&db_dir)?;`
   ...
   Also add `kronk_core::db` to the `use` imports at the top of the file. Add `use kronk_core::models::update;` for the update module.
...
- [ ] Test manually:
  - `kronk model update --check` (dry-run on installed models)
  - `kronk model update --refresh` (stamp metadata for existing models)
  - `kronk model update <specific-model>` (single model check)
  - `kronk model update --check --refresh` (should error due to conflicts_with)
- [ ] Run `cargo fmt --all && cargo clippy --workspace -- -D warnings`
- [ ] Run `cargo test --workspace`, confirm all tests pass
- [ ] Commit with message: "feat: add kronk model update command for checking and downloading model updates"

**Acceptance criteria:**
- [ ] `kronk model update` checks all installed models and displays status
- [ ] `kronk model update <model>` checks a single model
...
- [ ] `kronk model rm` cleans up DB records (best-effort)
- [ ] All tests pass, clippy clean, builds on workspace
