...
3. `--profile chat` is a kronk concept — llama-server doesn't understand it.
...
4. **Preserve raw backend args** — any flags that aren't kronk-specific pass through to `ModelConfig.args` unchanged.
...
| `--port <value>` | `ModelConfig.port` | Always extracted — kronk-managed port assignment. |
| `--ctx <value>` | `ModelConfig.context_length` | Always extracted — kronk injects `-c` at runtime from this or the model card. |
...
3. `--model` referencing a nonexistent model card → error: "Model 'X' not found. Run `kronk model pull` first."
4. `--model` value has `.gguf` extension → treated as backend file path, left in args
5. `server edit` with no kronk flags → only updates backend/args, preserves existing model/quant/profile
