...
`tama server add` and `tama server edit` treat all arguments after the backend path as raw backend args, dumping them into `ModelConfig.args`. This means tama-specific flags like `--model`, `--profile`, and `--quant` are passed directly to the backend (e.g. llama-server) instead of being stored in the proper `ModelConfig` struct fields.

Example command:
```bash
tama server add qwen35 /path/to/llama-server --model unsloth/Qwen3.5-0.8B --profile chat
```
...
The correct config (which `tama model create` produces) should be:
```toml
[models.qwen35]
backend = "llama_server"
...
3. `--profile chat` is a tama concept — llama-server doesn't understand it.
4. Context length (`-c`) and GPU layers (`-ngl`) from the model card are not injected.

## Goals

1. **Extract tama flags** from the raw arg vec in `server add` and `server edit`, setting the proper `ModelConfig` struct fields.
...
4. **Preserve raw backend args** — any flags that aren't tama-specific pass through to `ModelConfig.args` unchanged.
...
A new helper function `extract_tama_flags(args: Vec<String>) -> ExtractedFlags` will parse the raw arg vec and separate tama-specific flags from backend-passthrough args.

**Tama-specific flags to extract:**
...
| `--port <value>` | `ModelConfig.port` | Always extracted — tama-managed port assignment. |
| `--ctx <value>` | `ModelConfig.context_length` | Always extracted — tama injects `-c` at runtime from this or the model card. |
...
- Error with helpful messages if either is missing (e.g. "Run `tama model pull ...` first")

### Server Edit Behavior

For `server edit`, extracted flags **overwrite** existing `ModelConfig` fields. If a tama flag is not provided in the new command, the existing value is preserved (not cleared to `None`). This lets users run `tama server edit mymodel llama-server --profile coding` to change just the profile without re-specifying `--model` and `--quant`.

Implementation: `cmd_server_edit` must load the existing `ModelConfig` first, call `extract_tama_flags()` on the new command, then selectively merge — only updating fields that were explicitly provided. The current implementation overwrites `backend` and `args` wholesale; the new implementation must preserve `model`, `quant`, `profile`, `port`, `context_length`, and `source` unless explicitly overridden.

### Port Handling

`--port` is extracted into `ModelConfig.port`, which tama uses for port assignment when starting the backend. The `build_full_args()` function does **not** inject `--port` into backend args — tama manages port binding separately (the proxy routes to backends by port). Extracting `--port` prevents it from being passed as a raw arg to the backend, which is correct since tama controls port allocation.

### Source Field

When `--model` is extracted as a model card reference, `ModelConfig.source` is also set to the model ID (matching `model create` behavior). This provides consistency for `tama status` display.
...
Done.
  ...
  Model:     unsloth/Qwen3.5-0.8B
  Quant:     Q4_K_M
  GGUF:      /home/daniel/.config/tama/models/unsloth/Qwen3.5-0.8B/Qwen3.5-0.8B-Q4_K_M.gguf
  Profile:   chat
...
3. `--model` referencing a nonexistent model card → error: "Model 'X' not found. Run `tama model pull` first."
4. `--model` value has `.gguf` extension → treated as backend file path, left in args
5. `server edit` with no tama flags → only updates backend/args, preserves existing model/quant/profile
