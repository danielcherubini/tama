...
**Goal:** When tama builds llama.cpp or ik_llama from source with `GpuType::RocM`, emit the full set of recommended ROCm/HIP cmake flags (detected `AMDGPU_TARGETS`, rocWMMA flash attention, all KV-cache FA quants, libcurl) and export `HIPCXX`/`HIP_PATH` so the HIP toolchain is picked up reliably.

**Architecture:** Detection lives in `crates/tama-core/src/gpu.rs` as a pure function that shells out to `rocminfo` (with a `TAMA_AMDGPU_TARGETS` env override). Flag assembly stays in `build_cmake_args` in `crates/tama-core/src/backends/installer/source.rs` — the function's signature grows an `amdgpu_targets: Option<&[String]>` parameter so it stays pure and unit-testable. The Linux cmake subprocess gets `HIPCXX`/`HIP_PATH` from `hipconfig` injected into its env at the call site in `configure_cmake`.
...
Current state in `crates/tama-core/src/backends/installer/source.rs:331-333` (inside `build_cmake_args` which starts at line 306):
...
Env override rules:
- Read `TAMA_AMDGPU_TARGETS` via `std::env::var`. If present and non-empty after trim, split on `,` or `;`, trim each piece, drop empty pieces, return the resulting `Vec<String>` without running `rocminfo`. Do not validate the shape — the user is asserting they know what they want.
- If the env var is unset or empty, fall through to `rocminfo`.

**Steps:**
- [ ] Write failing tests in the existing `#[cfg(test)] mod tests` block at the bottom of `crates/tama-core/src/gpu.rs`.

  Because Rust's default test harness is multi-threaded and the env-override tests mutate a shared process-global env var (`TAMA_AMDGPU_TARGETS`), serialize them with a `static Mutex<()>` in the test module. Do NOT add the `serial_test` crate — stay dep-free. Put this near the top of `mod tests`:
  ...
  Each env-override test must take `let _guard = ENV_LOCK.lock().unwrap();` as its first line, and each must `std::env::remove_var("TAMA_AMDGPU_TARGETS")` at its start (defensive, in case a prior run panicked) and also at its end.
  ...
  - `test_detect_amdgpu_targets_env_override_semicolons` (uses `ENV_LOCK`): `std::env::set_var("TAMA_AMDGPU_TARGETS", "gfx1100;gfx1201")` then call → returns `vec!["gfx1100", "gfx1201"]`. Clean up with `remove_var`.
...
- [ ] Run `cargo test -p tama-core --lib gpu::tests::test_parse_rocminfo_gfx_names_single_gpu`
  - Did it fail with "function not found" / "unresolved import"? If it passed unexpectedly, stop and investigate why.
- [ ] Implement `parse_rocminfo_gfx_names` and `detect_amdgpu_targets` in `crates/tama-core/src/gpu.rs`. Place them after `detect_cuda_version_nvidia_smi` (around line 325) to keep vendor-detection helpers grouped. Add brief rustdoc per the signature above.
- [ ] Run `cargo test -p tama-core --lib gpu::tests`
...
- [ ] Run `cargo clippy -p tama-core --lib -- -D warnings`
...
**Acceptance criteria:**
- [ ] `pub fn detect_amdgpu_targets() -> Vec<String>` exported from `tama_core::gpu`.
...
**Steps:**
- [ ] Write failing tests in the `mod tests` block at the bottom of `crates/tama-core/src/backends/installer/source.rs`:
...
- [ ] Run `cargo test -p tama-core --lib backends::installer::source::tests::test_rocm_emits_full_flag_set`
...
- [ ] Run `cargo test -p tama-core --lib backends::installer::source::tests`
...
**Steps:**
- [ ] Write failing tests in the `mod tests` block of `crates/tama-core/src/backends/installer/source.rs`. Wrap each test body with `#[cfg(not(target_os = "windows"))]` to match the helper's cfg-gate:
...
- [ ] Run `cargo test -p tama-core --lib backends::installer::source::tests::test_hip_env_from_hipconfig_output_happy_path`
...
- [ ] Grep `rg -n "GGML_HIP=ON" crates/tama-core/src/backends/installer/source.rs` shows the flag only inside the `GpuType::RocM` branch.
- [ ] Grep `rg -n "AMDGPU_TARGETS" crates/tama-core/src/backends/installer/source.rs` shows the `format!` inside the ROCm branch.
- [ ] `rg -n "detect_amdgpu_targets" crates/tama-core` finds the definition in `gpu.rs` and the call in `source.rs`.
- [ ] `rg -n "HIPCXX" crates/tama-core` finds the env injection in `configure_cmake`.
