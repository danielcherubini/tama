...
**Files:**
- Modify: `crates/tama-cli/src/main.rs` (1982 → ~15 lines)
- Modify: `crates/tama-cli/src/lib.rs` (1989 → ~100 lines, becomes module root)
- Create: `crates/tama-cli/src/cli.rs` (~110 lines — Args, Commands, all clap enums)
- Create: `crates/tama-cli/src/flags.rs` (~110 lines — ExtractedFlags, extract_tama_flags)
- Create: `crates/tama-cli/src/service.rs` (~320 lines — Windows service dispatch, win_service_main, statics)
- Create: `crates/tama-cli/src/handlers/mod.rs` (re-exports)
- Create: `crates/tama-cli/src/handlers/run.rs` (~70 lines — cmd_run, build_full_args)
- Create: `crates/tama-cli/src/handlers/serve.rs` (~60 lines — cmd_serve, cmd_proxy, start_proxy_server)
- Create: `crates/tama-cli/src/handlers/status.rs` (~120 lines — cmd_status, format_duration_secs)
- Create: `crates/tama-cli/src/handlers/server.rs` (~250 lines — cmd_server, cmd_server_ls/add/edit/rm, resolve_backend)
- Create: `crates/tama-cli/src/handlers/config.rs` (~25 lines — cmd_config)
- Create: `crates/tama-cli/src/handlers/profile.rs` (~230 lines — cmd_profile)
- Create: `crates/tama-cli/src/handlers/service_cmd.rs` (~100 lines — cmd_service, service_start/stop_inner)
- Create: `crates/tama-cli/src/handlers/logs.rs` (~40 lines — cmd_logs)
- Test: `crates/tama-cli/tests/tests.rs` (existing — must still pass)

**Steps:**
- [ ] Verify all existing tests pass (`cargo test --package tama-cli`)
...
- [ ] Create `flags.rs` — move `ExtractedFlags` struct and `extract_tama_flags()` from lib.rs
...
- [ ] Reduce `main.rs` to: `use tama_cli::main as cli_main; fn main() { cli_main() }` (or equivalent thin wrapper)
- [ ] Delete all duplicated code from `main.rs`
- [ ] Run `cargo test --package tama-cli` — verify all tests pass
- [ ] Run `cargo clippy --package tama-cli -- -D warnings` — no warnings
- [ ] Commit: `refactor: consolidate CLI main.rs/lib.rs duplication into modules`

---

## Task 2: Split `crates/tama-core/src/config.rs` (1135 lines)

Convert `config.rs` into a `config/` directory module.

**Files:**
- Remove: `crates/tama-core/src/config.rs`
- Create: `crates/tama-core/src/config/mod.rs` (~30 lines — module declarations, re-exports)
- Create: `crates/tama-core/src/config/types.rs` (~170 lines — Config, ProxyConfig, General, BackendConfig, HealthCheck, ModelConfig, Supervisor structs + their derives)
- Create: `crates/tama-core/src/config/defaults.rs` (~90 lines — all `default_*()` serde helper functions, Default impl for ProxyConfig)
- Create: `crates/tama-core/src/config/loader.rs` (~280 lines — Config impl: load, save, save_to, load_from, path helpers: base_dir, config_dir, config_path, profiles_dir, configs_dir, models_dir, logs_dir, with_models_dir)
- Create: `crates/tama-core/src/config/resolve.rs` (~220 lines — Config impl: resolve_server, resolve_servers_for_model, resolve_health_url, resolve_backend_url, resolve_health_check, resolve_profile_params, build_args, build_full_args, effective_sampling, effective_sampling_with_card, proxy_url, service_name)
- Create: `crates/tama-core/src/config/migrate.rs` (~50 lines — migrate_model_cards_to_configs)
- Test: existing tests remain in `config/mod.rs` or a `config/tests.rs` submodule

**Steps:**
- [ ] Verify all existing tests pass (`cargo test --package tama-core -- config`)
...
- [ ] Run `cargo build --workspace` — verify no breakage in tama-cli
- [ ] Commit: `refactor: split config.rs into config/ module directory`

---

## Task 3: Split `crates/tama-core/src/proxy.rs` (876 lines) into `proxy/` directory
...
**Files:**
- Remove: `crates/tama-core/src/proxy.rs`
- Create: `crates/tama-core/src/proxy/mod.rs` (~20 lines — module declarations, re-exports)
- Create: `crates/tama-core/src/proxy/state.rs` (~250 lines — ModelState enum + impl, ProxyMetrics struct, ProxyState struct + query methods: new, is_model_loaded, get_model_state, get_backend_url, get_backend_pid, get_circuit_breaker_failures, get_available_server_for_model, update_last_accessed, get_model_card, build_status_response)
- Create: `crates/tama-core/src/proxy/lifecycle.rs` (~300 lines — ProxyState impl: load_model, unload_model, check_idle_timeouts — the heavy async model lifecycle logic)
- Create: `crates/tama-core/src/proxy/process.rs` (~110 lines — override_arg, is_process_alive, kill_process, force_kill_process, check_health)
- Existing: `crates/tama-core/src/proxy/server.rs` (719 lines — stays as-is for now, split in Task 4)

**Steps:**
- [ ] Verify tests pass (`cargo test --package tama-core -- proxy`)
...
## Task 4: Split `crates/tama-core/src/proxy/server.rs` (719 lines)

Split the Axum proxy server into server setup vs. route handlers.

**Files:**
- Modify: `crates/tama-core/src/proxy/server.rs` (719 → ~100 lines — ProxyServer struct + impl, router construction)
- Create: `crates/tama-core/src/proxy/handlers.rs` (~250 lines — all handle_* functions, json_error_response)
- Create: `crates/tama-core/src/proxy/forward.rs` (~240 lines — forward_request function)
- Modify: `crates/tama-core/src/proxy/mod.rs` (add new module declarations)
...
## Task 5: Split `crates/tama-core/src/backends/installer.rs` (667 lines)

Split the installer into logical phases: URL resolution, downloading, extraction, and installation.

**Files:**
- Modify: `crates/tama-core/src/backends/installer.rs` (667 → ~120 lines — InstallOptions, install_backend, prepare_target_dir, install_prebuilt)
- Create: `crates/tama-core/src/backends/download.rs` (~50 lines — download_file with progress bar)
- Create: `crates/tama-core/src/backends/extract.rs` (~130 lines — extract_archive, find_backend_binary)
- Create: `crates/tama-core/src/backends/source_build.rs` (~270 lines — install_from_source, cmake build logic)
- Create: `crates/tama-core/src/backends/urls.rs` (~80 lines — get_prebuilt_url)
- Modify: `crates/tama-core/src/backends/mod.rs` (add new module declarations)

**Steps:**
- [ ] Verify tests pass (`cargo test --package tama-core -- backends`)
...
## Task 6: Split `crates/tama-core/src/models/download.rs` (518 lines)

Split the download module by download strategy.

**Files:**
- Modify: `crates/tama-core/src/models/download.rs` (518 → ~130 lines — download_chunked entry point, build_client, cleanup_temp_files, constants)
- Create: `crates/tama-core/src/models/download_single.rs` (~135 lines — download_single function)
- Create: `crates/tama-core/src/models/download_parallel.rs` (~210 lines — download_parallel, download_chunk_with_retry)
- Modify: `crates/tama-core/src/models/mod.rs` (add new module declarations)

**Steps:**
- [ ] Verify tests pass (`cargo test --package tama-core -- models::download`)
