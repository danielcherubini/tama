...
**Goal:** Migrate the backend registry from a `backend_registry.toml` file on disk to the SQLite database (`tama.db`), while keeping `config.toml` as the sole place where named backends are declared (with exactly `llama_cpp` and `ik_llama` as the two well-known names).
...
**Tech Stack:** Rust, SQLite via `rusqlite`, existing `crate::db` migration system, existing `tama-core` and `tama-cli` crates.
...
**Steps:**
- [ ] Write a failing test in `crates/tama-core/src/db/mod.rs` `#[cfg(test)]` block: call `open_in_memory()`, then assert that `backend_installations` table exists in `sqlite_master`. Name the test `test_migration_v3_creates_backend_installations`.
- [ ] Run `cargo test --package tama-core test_migration_v3_creates_backend_installations -- --nocapture`
...
- [ ] Also verify existing migration tests still pass: `cargo test --package tama-core -- db`
...
**Context:**
Following the existing pattern in `crates/tama-core/src/db/queries.rs`, we need typed query functions for the new `backend_installations` table. These are pure data access functions — no business logic. They will be called by the refactored `BackendRegistry` in a later task.
...
**Steps:**
- [ ] Write failing tests in `crates/tama-core/src/db/queries.rs` `#[cfg(test)]` block:
...
- [ ] Run `cargo test --package tama-core -- queries::tests::test_insert_and_get_active_backend`
...
- `crates/tama-core/src/backends/registry/registry_ops.rs` — full rewrite of `BackendRegistry`
- `crates/tama-core/src/backends/registry/mod.rs` — update re-exports
- `crates/tama-core/src/backends/registry/backup.rs` — remove module entirely
- `crates/tama-core/src/backends/mod.rs` — remove `RegistryData` from re-exports if present
- `crates/tama-core/src/backends/updater.rs` — adjust for `get()` return type change
- `crates/tama-core/Cargo.toml` — add `serde_json` if not present

**Affected files for CLI update:**
- `crates/tama-cli/src/commands/backend.rs` — update all `BackendRegistry` usage
...
- `BackendRegistry::open(config_dir: &Path) -> Result<Self>` — takes the **config directory** (NOT the .db file path), calls `crate::db::open(config_dir)` which internally opens `<config_dir>/tama.db`. This matches the convention of `crate::db::open`.
...
In `crates/tama-core/src/backends/updater.rs`, the `update_backend` function calls `registry.get(backend_name)`. Change it from:
```rust
registry
    .get(backend_name)
...
**Files:**
- Rewrite: `crates/tama-core/src/backends/registry/registry_ops.rs`
- Modify: `crates/tama-core/src/backends/registry/mod.rs`
- Delete contents of: `crates/tama-core/src/backends/registry/backup.rs` (replace with empty file is fine, or remove `pub mod backup;` from `mod.rs`)
...
**Steps:**
- [ ] Check `crates/tama-core/Cargo.toml` for `serde_json`; add it under `[dependencies]` if missing (e.g., `serde_json = { version = "1", features = ["std"] }`).
...
- [ ] Run `cargo test --package tama-core -- backends::registry::tests`
...
- [ ] Update `crates/tama-core/src/backends/mod.rs`: remove `RegistryData` re-export if present.
- [ ] Update `updater.rs` as described in Part C.
- [ ] Run `cargo build --package tama-core`
  - Did it succeed? If not, fix `tama-core` compilation errors before proceeding.
- [ ] Run `cargo test --package tama-core -- backends::registry`
  - Did all pass?
- [ ] Now update `crates/tama-cli/src/commands/backend.rs` as described in Part B.
...
Currently, `Config::default()` in `crates/tama-core/src/config/loader.rs` hardcodes only `llama_cpp`. We should also add `ik_llama` with sensible defaults.
...
- `crates/tama-cli/src/handlers/run.rs` — reads `backend.path` to get the binary path for spawning the process
- `crates/tama-cli/src/handlers/service_cmd.rs` — reads `backend.path`
- `crates/tama-cli/src/handlers/serve.rs` OR `service.rs` — may read `backend.path`
...
```rust
// In crates/tama-core/src/config/resolve.rs
...
**Files:**
- Modify: `crates/tama-core/src/config/types.rs` — make `BackendConfig.path` an `Option<String>`
- Modify: `crates/tama-core/src/config/loader.rs` — update `Config::default()` to include both `llama_cpp` and `ik_llama` (both with `path: None`); update existing `llama_cpp` entry from `path: String` to `path: None`
- Modify: `crates/tama-core/src/config/resolve.rs` — add `resolve_backend_path()` method; add `use rusqlite::Connection;`
- Modify: `crates/tama-cli/src/handlers/run.rs` — replace `backend.path` with `config.resolve_backend_path(&backend_name, &conn)?`
- Modify: `crates/tama-cli/src/handlers/service_cmd.rs` — same
...
**Steps:**
- [ ] Search for all uses of `backend.path` (as `BackendConfig`) in the codebase: `rg "backend\.path" crates/tama-cli/src/handlers/`. Note every file and line.
- [ ] Write failing tests in `crates/tama-core/src/config/resolve.rs` `#[cfg(test)]` block:
...
**Context:**
Users who already have a `backend_registry.toml` on disk need their existing backend entries migrated into the new `backend_installations` table in `tama.db`. This backfill should happen once at startup, detected by checking whether the `backend_registry.toml` file exists. After migrating, the file is renamed to `backend_registry.toml.migrated` so it is not re-imported.

First, read `crates/tama-core/src/db/backfill.rs` to understand the existing backfill pattern, and search for where `backfill` is called in the CLI (`rg "backfill" crates/tama-cli/`).
...
**Steps:**
- [ ] Read `crates/tama-core/src/db/backfill.rs` to understand the existing pattern.
- [ ] Run `rg "backfill" crates/tama-cli/src/` to find the call site(s).
