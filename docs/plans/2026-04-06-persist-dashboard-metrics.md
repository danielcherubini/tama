...
**Status:** ✅ COMPLETED - See git commits `b657e22` ("feat: persist dashboard metrics to SQLite + stream via SSE (#34)"), `8e6a5b5` ("feat(web): stream dashboard metrics over SSE instead of polling"), `fd12bf8` ("feat(api): add GET /tama/v1/system/metrics/stream SSE endpoint"), `4c6d6e2` ("feat(proxy): persist + broadcast system metrics every 2s"), `2892764` ("feat(db): add system_metrics_history table (migration v4) with insert+prune helpers")

**Architecture:** The existing 5-second background metrics refresh task (`ProxyServer::new` in `proxy/server/mod.rs`) is dropped to a 2-second cadence and extended to (a) write each sample to a new `system_metrics_history` SQLite table, (b) prune rows older than `proxy.metrics_retention_secs` in the same transaction, and (c) broadcast the sample over a `tokio::sync::broadcast` channel held in `ProxyState`. A new SSE endpoint `GET /tama/v1/system/metrics/stream` subscribes to that channel and forwards samples to clients. The Leptos dashboard replaces its `setInterval` polling with a `web_sys::EventSource` and appends streamed samples to its existing `RwSignal<Vec<...>>` ring buffer. Decisions confirmed by user: configurable retention via `proxy.metrics_retention_secs` (default 24h), 2-second cadence, inline pruning, no historical backfill in the SSE response, full schema (`SystemMetrics` + `models_loaded`), silent no-op when `db_dir` is `None`, and `/tama/v1/system/health` left untouched.

**Tech Stack:** Rust, axum (SSE via `axum::response::sse`), `async-stream` 0.3 (already in `tama-core/Cargo.toml`), `rusqlite` (existing `db` module), `tokio::sync::broadcast`, Leptos 0.7 CSR, `web-sys::EventSource`.
...
- [ ] Write a failing unit test in `crates/tama-core/src/config/types.rs` (in the existing `#[cfg(test)] mod tests` block, or add one if absent) named `test_proxy_config_default_metrics_retention` that constructs `ProxyConfig::default()` and asserts `metrics_retention_secs == 86_400`.
- [ ] Run `cargo test --package tama-core test_proxy_config_default_metrics_retention`
...
- [ ] Run `cargo test --package tama-core --lib config::types::tests`
...
2. **New record type and queries** in `crates/tama-core/src/db/queries.rs`. Add at the end of the "Record types" section (after `BackendInstallationRecord`):
   ```rust
...
3. **Migration test** in `crates/tama-core/src/db/mod.rs::tests`:
   ```rust
   #[test]
...
**Steps:**
- [ ] Write a failing test `test_migration_v4_creates_system_metrics_history` in `crates/tama-core/src/db/mod.rs::tests`.
- [ ] Run `cargo test --package tama-core test_migration_v4_creates_system_metrics_history`
  - Did it fail because the table doesn't exist? If it passed unexpectedly, stop and investigate.
- [ ] Bump `LATEST_VERSION` to 4 and add the migration tuple in `crates/tama-core/src/db/migrations.rs`.
- [ ] Re-run `cargo test --package tama-core test_migration_v4_creates_system_metrics_history`
  - Did it pass? If not, fix and re-run.
- [ ] Write failing tests in `crates/tama-core/src/db/queries.rs::tests`:
...
- [ ] Run `cargo test --package tama-core --lib db::queries::tests`
  - Did all three tests fail to compile (missing functions)? If they passed, stop and investigate.
- [ ] Add `SystemMetricsRow` and the three helper functions to `crates/tama-core/src/db/queries.rs`.
- [ ] Re-run `cargo test --package tama-core --lib db::queries::tests`
...
**Files:**
- Modify: `crates/tama-core/src/gpu.rs` (add `MetricSample` near `SystemMetrics`)
- Modify: `crates/tama-core/src/proxy/types.rs` (add `metrics_tx` field to `ProxyState`)
- Modify: `crates/tama-core/src/proxy/state.rs` (initialize `metrics_tx` in `ProxyState::new`)

**What to implement:**

1. **`MetricSample` struct** in `crates/tama-core/src/gpu.rs`, immediately below `SystemMetrics`:
   ```rust
...
2. **Add field to `ProxyState`** in `crates/tama-core/src/proxy/types.rs` (around line 134):
   ```rust
...
3. **Initialize in `ProxyState::new`** in `crates/tama-core/src/proxy/state.rs`. Add inside the constructor (before the struct literal):
   ```rust
...
**What NOT to change:**
- Do not touch the existing `system_metrics: Arc<RwLock<SystemMetrics>>` field — it's still used by `/tama/v1/system/health`.
...
**Context:**
`ProxyServer::new` in `crates/tama-core/src/proxy/server/mod.rs` currently spawns a Tokio task that calls `crate::gpu::collect_system_metrics_with` every 5 seconds and writes the result to `state.system_metrics`. We extend this task to:
...
The DB write is wrapped in `tokio::task::spawn_blocking` because rusqlite is synchronous and we don't want to stall the metrics loop on a slow flush. Persistence and broadcast are both best-effort: failures (no DB, no subscribers, lagged subscribers) are logged at `warn!` level but never propagate. The existing `state.system_metrics.write()` update stays — it's still the source for the `/tama/v1/system/health` snapshot endpoint, which we are explicitly leaving untouched (per user decision).
...
**Steps:**
- [ ] Write a failing async test in `crates/tama-core/src/proxy/server/mod.rs::tests` named `test_metrics_task_persists_to_db`. The test:
...
- [ ] Run `cargo test --package tama-core test_metrics_task_persists_to_db test_metrics_task_broadcasts_samples`
...
- [ ] Re-run `cargo test --package tama-core test_metrics_task_persists_to_db test_metrics_task_broadcasts_samples`
...
- [ ] `state.system_metrics` cached snapshot is still updated (so `/tama/v1/system/health` keeps working).
- [ ] When `db_dir` is `None`, the loop still ticks and broadcasts (silent no-op for persistence).

---

## Task New SSE endpoint `GET /tama/v1/system/metrics/stream`
...
This mirrors the existing `handle_pull_job_stream` in `crates/tama-core/src/proxy/tama_handlers.rs:736` (which uses `Sse::new(stream).keep_alive(KeepAlive::default())`), except we use `async_stream::stream!` instead of `futures::stream::unfold` because we have a single-phase loop with no per-iteration state.

**Files:**
- Modify: `crates/tama-core/src/proxy/tama_handlers.rs` (new handler)
- Modify: `crates/tama-core/src/proxy/server/router.rs` (route registration)

**What to implement:**

1. **Handler** in `crates/tama-core/src/proxy/tama_handlers.rs`. Add at the end of the file (or just below `handle_tama_system_health`):
...
2. **Route registration** in `crates/tama-core/src/proxy/server/router.rs`:
   - Add `handle_system_metrics_stream` to the `use crate::proxy::tama_handlers::{...};` list (the import block at line 12).
   - Add the route to `build_router` immediately after the existing `/tama/v1/system/health` line (line 48):
     ```rust
     .route("/tama/v1/system/metrics/stream", get(handle_system_metrics_stream))
     ```

**What NOT to change:**
- Do not modify `handle_tama_system_health` or its response struct (per user decision).
...
**Steps:**
- [ ] Write a failing async integration test in `crates/tama-core/src/proxy/server/mod.rs::tests` named `test_system_metrics_stream_emits_samples`. The test:
  ...
  3. Uses `reqwest::Client` to `GET /tama/v1/system/metrics/stream` with a 5-second timeout.
...
- [ ] Run `cargo test --package tama-core test_system_metrics_stream_emits_samples`
  - Did it fail because the route returns 404? If it passed, stop and investigate.
- [ ] Add the handler to `crates/tama-core/src/proxy/tama_handlers.rs`.
- [ ] Wire the route in `crates/tama-core/src/proxy/server/router.rs`.
- [ ] Re-run `cargo test --package tama-core test_system_metrics_stream_emits_samples`
  - Did it pass? If it times out, double-check that Task 4 is committed and the metrics task is actually running.
- [ ] Run `cargo fmt --all`
- [ ] Run `cargo clippy --workspace -- -D warnings`
- [ ] Run `cargo test --workspace`
  - Did all tests pass? If not, fix and re-run.
- [ ] Commit with message: `feat(api): add GET /tama/v1/system/metrics/stream SSE endpoint`

**Acceptance criteria:**
- [ ] `GET /tama/v1/system/metrics/stream` returns `text/event-stream` and emits `sample` events.
...
- [ ] `/tama/v1/system/health` still works exactly as before.
...
**Context:**
The Tama management API is documented in `docs/openapi/tama-api.yaml`. The existing `/tama/v1/system/health` entry lives around line 301. We add a peer entry for `/tama/v1/system/metrics/stream` and a `MetricSample` schema. This task is independently committable: it touches only docs and has no Rust code dependencies (the schema is hand-written to match Task 3's struct).

**Files:**
- Modify: `docs/openapi/tama-api.yaml`

**What to implement:**

1. Under `paths:`, immediately after the `/tama/v1/system/health` block (around line 337), add:
...
**What NOT to change:**
- Do not modify the existing `SystemHealth` schema or `/tama/v1/system/health` path.

**Steps:**
- [ ] Run `ls docs/openapi/` and inspect `tama-api.yaml` to locate the `paths:` and `components.schemas:` sections.
- [ ] Add the new path entry after `/tama/v1/system/health`.
...
- [ ] If a workspace has an OpenAPI linter (search for `redocly`, `spectral`, or `openapi` in `Makefile` and root config files), run it. Otherwise, validate manually with a YAML parser:
  ```bash
  python3 -c "import yaml; yaml.safe_load(open('docs/openapi/tama-api.yaml'))"
  ```
  - Did it parse without error? If not, fix YAML syntax and re-run.
- [ ] Commit with message: `docs(openapi): document /tama/v1/system/metrics/stream and MetricSample`

**Acceptance criteria:**
- [ ] `tama-api.yaml` parses as valid YAML.
- [ ] `/tama/v1/system/metrics/stream` is documented with `text/event-stream` content type.
...
**Context:**
Today, `crates/tama-web/src/pages/dashboard.rs` polls `/tama/v1/system/health` every 3 seconds via `web_sys::set_interval` and accumulates a 100-entry in-memory ring buffer. We replace this with a `web_sys::EventSource` connection to `/tama/v1/system/metrics/stream`. Each `sample` event is parsed into a new local `MetricSample` struct (mirror of the backend struct from Task 3) and pushed into the existing `history` `RwSignal`. The 100-entry cap stays — at 2s cadence that's ~3.3 minutes of live data. The status badge in the page header continues to derive from the most recent sample (per user decision, we hard-code `"ok"` as the status string since the existence of recent samples implies health). The "Restart" button is unchanged. The error/retry UX is preserved: if `EventSource.onerror` fires AND the history buffer is empty, we show the existing failure card with a Retry button that re-creates the EventSource.

`web-sys` features must be expanded — currently `tama-web/Cargo.toml` line 13 only enables `Window`, `Document`, `HtmlElement`, `HtmlInputElement`. We need `EventSource`, `MessageEvent`, and `Event` (for the error handler closure type).

**Files:**
- Modify: `crates/tama-web/Cargo.toml` (add `web-sys` features)
- Modify: `crates/tama-web/src/pages/dashboard.rs` (replace polling logic)

**What to implement:**

1. **`web-sys` features** in `crates/tama-web/Cargo.toml` line 13. Change to:
   ```toml
...
2. **`MetricSample` and `VramInfo` structs** in `crates/tama-web/src/pages/dashboard.rs`. Replace the existing `SystemHealth` and `VramInfo` structs with:
   ```rust
   #[derive(Debug, Clone, Serialize, Deserialize)]
   struct MetricSample {
       ts_unix_ms: i64,
   ...
   pub fn Dashboard() -> impl IntoView {
       ...
       let restart: Action<(), (), LocalStorage> =
           Action::new_unsync(|_: &()| async move {
               let _ = gloo_net::http::Request::post("/tama/v1/system/restart")
                   .send()
                   .await;
           });
...
4. **Remove**:
   ...
   - The `LocalResource::new` block that fetches `/tama/v1/system/health`.
...
**What NOT to change:**
- Do not touch `crates/tama-web/src/components/sparkline.rs`.
- Do not touch `crates/tama-web/style.css`.
- Do not touch any other page under `crates/tama-web/src/pages/`.

**Steps:**
- [ ] Update `crates/tama-web/Cargo.toml` line 13 to add the new `web-sys` features.
- [ ] Run `cargo check --package tama-web` to confirm the feature names are valid.
...
- [ ] Run `cargo check --package tama-web`
  - Did it succeed? If not, fix and re-run.
- [ ] Run `cargo clippy --package tama-core --package tama-cli --package tama-mock -- -D warnings`
  - Did it succeed? If not, fix and re-run.
- [ ] Run `cargo test --workspace`
  - Did all tests pass? If not, fix and re-run.
- [ ] Manually verify (if `trunk serve` is available): start the tama proxy, then `cd crates/tama-web && trunk serve`, open the dashboard, confirm graphs populate within ~2 seconds and continue updating.
- [ ] Commit with message: `feat(web): stream dashboard metrics over SSE instead of polling`

**Acceptance criteria:**
- [ ] Dashboard opens an `EventSource` to `/tama/v1/system/metrics/stream` on mount.
...
| File | Task | Change |
|---|---|---|
| `crates/tama-core/src/config/types.rs` | 1 | Add `metrics_retention_secs` field + default |
| `config/tama.toml*` | 1 | Document the new key |
| `crates/tama-core/src/db/migrations.rs` | 2 | Migration v4, bump `LATEST_VERSION` |
| `crates/tama-core/src/db/queries.rs` | 2 | `SystemMetricsRow` + 3 helpers + tests |
| `crates/tama-core/src/db/mod.rs` | 2 | Migration v4 test |
| `crates/tama-core/src/gpu.rs` | 3 | `MetricSample` struct |
| `crates/tama-core/src/proxy/types.rs` | 3 | Add `metrics_tx` field to `ProxyState` |
| `crates/tama-core/src/proxy/state.rs` | 3 | Initialize `metrics_tx` in `new` |
| `crates/tama-core/src/proxy/server/mod.rs` | 4 | 2s tick, persist + broadcast |
| `crates/tama-core/src/proxy/tama_handlers.rs` | 5 | `handle_system_metrics_stream` handler |
| `crates/tama-core/src/proxy/server/router.rs` | 5 | Wire `/tama/v1/system/metrics/stream` |
| `docs/openapi/tama-api.yaml` | 6 | Document endpoint + `MetricSample` schema |
| `crates/tama-web/Cargo.toml` | 7 | Add `EventSource`/`MessageEvent`/`Event`/`EventSourceInit` web-sys features |
| `crates/tama-web/src/pages/dashboard.rs` | 7 | Replace polling with `EventSource` |
