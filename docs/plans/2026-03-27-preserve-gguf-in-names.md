...
**Goal:** Preserve `-GGUF` / `-gguf` in model IDs and directory paths instead of stripping it during `tama model pull`.
...
**Files:**
- Modify: `crates/tama-cli/src/commands/model.rs` (lines 96-110)
...
**Note:** `fetch_community_card()` in `crates/tama-core/src/models/pull.rs` is NOT changed — it tries the exact repo name first, then falls back to stripped variants for URL lookup. This is correct behavior since community card files on GitHub may be named without `-GGUF`.
