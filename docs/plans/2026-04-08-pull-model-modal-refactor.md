...
**Tech Stack:** Rust, Leptos 0.7, WASM frontend (`crates/tama-web`)

**Reference implementation:** `crates/tama-web/src/pages/model_editor.rs` lines 1356–1365 shows the exact pattern: a `Modal` wrapping `PullQuantWizard` with `initial_repo`, `is_open`, `on_complete`, and `on_close` props.
...
**Files:**
- Delete: `crates/tama-web/src/pages/pull.rs`
...
1. **Delete `crates/tama-web/src/pages/pull.rs`** entirely. It is a 16-line file that just renders `PullQuantWizard` on a page — no logic to preserve.

2. **`crates/tama-web/src/pages/mod.rs`** — remove `pub mod pull;`. The file currently has 6 lines, one per page module. After the change it should have 5 lines.

3. **`crates/tama-web/src/lib.rs`** — remove the route:
   ```rust
...
4. **`crates/tama-web/src/components/nav.rs`** — remove the nav link:
   ```rust
...
**Steps:**
- [ ] Delete `crates/tama-web/src/pages/pull.rs`
- [ ] Remove `pub mod pull;` from `crates/tama-web/src/pages/mod.rs`
- [ ] Remove the `/pull` route from `crates/tama-web/src/lib.rs`
- [ ] Remove the "Pull Model" nav link from `crates/tama-web/src/components/nav.rs`
- [ ] Run `cargo build -p tama-web`
...
- [ ] Run `cargo clippy -p tama-web -- -D warnings`
  - Did it succeed? Fix any warnings before continuing.
- [ ] Run `cargo fmt --all`
- [ ] Commit with message: `feat: remove /pull page, route, and nav link`

**Acceptance criteria:**
- [ ] `crates/tama-web/src/pages/pull.rs` no longer exists
- [ ] `crates/tama-web/src/pages/mod.rs` has no `pull` module
- [ ] `crates/tama-web/src/lib.rs` has no `/pull` route
- [ ] `crates/tama-web/src/components/nav.rs` has no "Pull Model" link
- [ ] `cargo build -p tama-web` succeeds with no errors
- [ ] `cargo clippy -p tama-web -- -D warnings` passes
...
- [ ] `cargo build -p tama-web` and `cargo clippy -p tama-web -- -D warnings` both succeed
