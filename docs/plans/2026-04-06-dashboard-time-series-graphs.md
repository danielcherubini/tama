...
**Architecture:** The existing 3-second polling loop on the dashboard already fetches `SystemHealth` from `/tama/v1/system/health`. We will accumulate these snapshots into a client-side ring buffer (max 100 entries ≈ 5 minutes at 3s intervals). A new reusable `<SparklineChart>` Leptos component will render the buffered data as responsive SVG area charts. No backend changes required — all history is kept in browser memory.
...
**Files:**
- Create: `crates/tama-web/src/components/sparkline.rs`
- Modify: `crates/tama-web/src/components/mod.rs` — add `pub mod sparkline;`
...
**Steps:**
- [ ] Create `crates/tama-web/src/components/sparkline.rs` with the `SparklineChart` component as described above
- [ ] Add `pub mod sparkline;` to `crates/tama-web/src/components/mod.rs`
- [ ] Run `cargo fmt --all`
- [ ] Run `cargo clippy --workspace -- -D warnings`
  - Did it succeed? The WASM target may require `--target wasm32-unknown-unknown` for the tama-web crate. If clippy fails on tama-web specifically, try `cargo clippy --package tama-core --package tama-cli --package tama-mock -- -D warnings` to at least validate the non-WASM crates, and visually verify the tama-web code compiles with `cargo build --package tama-web --target wasm32-unknown-unknown` (if the target is installed) or just `cargo check --package tama-web`.
...
**Steps:**
- [ ] Add the sparkline CSS rules to `crates/tama-web/style.css` after the gauge section (after line ~313)
...
**Context:**
This is the main task. The current `Dashboard` component polls `/tama/v1/system/health` every 3 seconds and displays the latest snapshot as gauge bars and numbers. We need to:
...
- [ ] Run `cargo fmt --all`
- [ ] Run `cargo clippy --workspace -- -D warnings` (or check tama-web specifically)
- [ ] Visually verify by building with Trunk if possible: `trunk serve` in `crates/tama-web/`
